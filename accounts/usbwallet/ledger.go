...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-seke/master/doc/sekeapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and sekechain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an sekechain transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whsekeer the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "sekechain app in browser mode", w.failure
	}
	if w.offline() {
		return "sekechain app offline", w.failure
	}
	return fmt.Sprintf("sekechain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whsekeer the wallet and the sekechain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// sekechain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the sekechain app's version, will fail prior to v1.0.2
...
// and returning the sekechain address located on that derivation path.
...
//
// Note, if the version of the sekechain application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonsekeeless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the sekechain app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the sekechain wallet app running
...
// ledgerDerive retrieves the currently active sekechain address from a Ledger
...
//   sekechain address length | 1 byte
//   sekechain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the sekechain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an sekechain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whsekeer legacy or EIP155 signing was requeste
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the sekechain signature and do a sanity validation
	if len(reply) != 65 {
