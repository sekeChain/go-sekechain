...
import (
	...
	"github.com/sekechain/go-sekechain/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whsekeer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whsekeer the Trezor is opened, closed
// or whsekeer the sekechain app was not started on it.
...
// sekechain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.sekechainAddress)
	if _, err := w.trezorExchange(&trezor.sekechainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.sekechainSignTx{
	...
	response := new(trezor.sekechainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.sekechainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
