...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the sekechain
...
type wallet struct {
	...
	deriveChain    sekeereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whsekeer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived sekechain account
			...
			// If the next account is empty, stop self-derivation, but add it nonsekeeless
...
// Contains implements accounts.Wallet, returning whsekeer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain sekeereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
