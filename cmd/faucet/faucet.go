...
// faucet is a sekeer faucet backed by a light client.
package main
...
var (
	...
	sekePortFlag = flag.Int("sekeport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the sekechain protocol")
	statsFlag   = flag.String("sekestats", "", "sekestats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of sekeers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for sekechain and the faucet")
)

var (
	sekeer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s sekeers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *sekePortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // sekechain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an sekechain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // sekechain protocol stack
	client *sekeclient.Client   // Client connection to the sekechain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		P2P: p2p.Config{
			...
			MaxPeers:         25,
			BoosekerapNodesV5: enodes,
		},
	})
	if err != nil {
		return nil, err
	}
	// Assemble the sekechain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := seke.DefaultConfig
	...
	// Assemble the sekestats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.Lightsekechain
			ctx.Service(&serv)
			return sekestats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := sekeclient.NewClient(api)
...
// close terminates the sekechain connection and tears down the faucet.
...
// apiHandler handles requests for sekeer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, sekeer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the sekechain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somsekeing funky happened, please open an issue at https://github.com/sekechain/go-sekechain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), sekeer)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, sekeer)
...
// the username, avatar URL and sekechain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for sekechain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No sekechain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the sekechain address and profile URL.
...
// returning the username, avatar URL and sekechain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain sekechain address,
