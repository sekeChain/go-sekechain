...
FROM sekeereum/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--sekestats", "{{.sekestats}}", "--sekeport", "{{.sekePort}}",     \
...
services:
  faucet:
    ...
    ports:
      - "{{.sekePort}}:{{.sekePort}}"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"sekestats":      config.node.sekestats,
		"sekePort":       config.node.port,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"sekePort":       config.node.port,
		"sekeName":       config.node.sekestats[:strings.Index(config.node.sekestats, ":")],
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
		"FaucetAmount":  config.amount,
		"FaucetMinutes": config.minutes,
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"sekechain listener port":       strconv.Itoa(info.node.port),
		"Funding amount (base tier)":   fmt.Sprintf("%d sekeers", info.amount),
		"Funding cooldown (base tier)": fmt.Sprintf("%d mins", info.minutes),
		"Funding tiers":                strconv.Itoa(info.tiers),
		"Captha protection":            fmt.Sprintf("%v", info.captchaToken != ""),
		"sekestats username":            info.node.sekestats,
	}
	if info.noauth {
...
// checkFaucet does a health-check against an faucet server to verify whsekeer
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			port:     infos.portmap[infos.envvars["seke_PORT"]+"/tcp"],
			sekestats: infos.envvars["seke_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
