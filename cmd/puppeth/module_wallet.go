...
FROM puppseke/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gseke --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gseke --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --sekestats \'{{.sekestats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PuppsekeNetworkID/{{.NetworkID}}/g' dist/js/sekeerwallet-master.js && \
	sed -i 's/PuppsekeNetwork/{{.Network}}/g'     dist/js/sekeerwallet-master.js && \
	sed -i 's/PuppsekeDenom/{{.Denom}}/g'         dist/js/sekeerwallet-master.js && \
	sed -i 's/PuppsekeHost/{{.Host}}/g'           dist/js/sekeerwallet-master.js && \
	sed -i 's/PuppsekeRPCPort/{{.RPCPort}}/g'     dist/js/sekeerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whsekeer
// it's running, and if yes, whsekeer it's responsive.
