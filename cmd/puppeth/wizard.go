...
// config contains all the configurations needed by puppseke that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	sekestats  string   // sekestats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppseke configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // sekechain services known to be running on servers
...
// readDefaulsekering reads a single line from stdin, trimming if from spaces. If
// an empty line is entered, the default value is returned.
func (w *wizard) readDefaulsekering(def string) string {
	fmt.Printf("> ")
	text, err := w.in.ReadString('\n')
	if err != nil {
		log.Crit("Failed to read user input", "err", err)
...
func (w *wizard) readDefaultBigInt(def *big.Int) *big.Int {
	for {
		...
		val, ok := new(big.Int).Sesekering(text, 0)
		if !ok {
...
func (w *wizard) readAddress() *common.Address {
	for {
		...
		bigaddr, _ := new(big.Int).Sesekering(text, 16)
...
// converts it to an sekechain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by sekestats, which compares IPs textually, not by value.
