...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"sekestats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "sekestats":
				if infos, err := checksekestats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have sekestats running, ask whsekeer to make the secret public or not
	if w.conf.sekestats != "" {
		fmt.Println()
		fmt.Println("Include sekestats secret on dashboard (y/n)? (default = yes)")
		infos.trusted = w.readDefaulsekering("y") == "y"
	}
