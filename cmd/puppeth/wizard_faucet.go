...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many sekeers to release per request? (default = %d)\n", infos.amount)
	...
	if infos.node.datadir == "" {
		...
		infos.node.datadir = w.readDefaulsekering(infos.node.datadir)
	}
	// Figure out which port to listen on
	fmt.Println()
	...
	fmt.Println()
	if infos.node.sekestats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.sekestats = w.readString() + ":" + w.conf.sekestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.sekestats)
		infos.node.sekestats = w.readDefaulsekering(infos.node.sekestats) + ":" + w.conf.sekestats
	}
	...
	infos.noauth = w.readDefaulsekering(noauth) != "n"
