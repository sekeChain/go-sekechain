...
// makeWizard creates and returns a new puppseke wizard.
...
// setting up a new or managing an existing sekechain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppseke, your sekechain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new sekechain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and sekestats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppseke uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering sekechain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppseke", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somsekeing I can do")
		}
	}
}
