...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for sekestats availability")
	if infos, err := checksekestats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["sekestats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["sekestats"] = infos.Report()
		sekestats = infos.config
	}
