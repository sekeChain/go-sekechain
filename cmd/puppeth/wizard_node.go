...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.sekestats == "" {
		log.Error("No sekestats server configured")
		return
	}
	...
	if infos.datadir == "" {
		...
		infos.datadir = w.readDefaulsekering(infos.datadir)
	}
	if w.conf.Genesis.Config.sekeash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			...
			infos.ethashdir = w.readDefaulsekering(infos.ethashdir)
		}
	}
	...
	if infos.sekestats == "" {
		...
		infos.sekestats = w.readString() + ":" + w.conf.sekestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.sekestats)
		infos.sekestats = w.readDefaulsekering(infos.sekestats) + ":" + w.conf.sekestats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.sekeash != nil {
			// sekeash based miners only need an sekeerbase to mine against
			fmt.Println()
			if infos.sekeerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.sekeerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.sekeerbase)
				infos.sekeerbase = w.readDefaultAddress(common.HexToAddress(infos.sekeerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy sekechain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
