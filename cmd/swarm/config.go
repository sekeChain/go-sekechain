...
const (
	...
	Gseke_ENV_DATADIR          = "Gseke_DATADIR"
)
...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", check github.com/sekechain/go-sekechain/swarm/api/config.go for available fields")
		}
...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (gseke --datadir)
...
func configFileOverride(config *bzzapi.Config, ctx *cli.Context) (*bzzapi.Config, error) {
	...
	//only do somsekeing if the -config flag has been set
...
func envVarsOverride(currentConfig *bzzapi.Config) (config *bzzapi.Config) {
	...
	if datadir := os.Getenv(Gseke_ENV_DATADIR); datadir != "" {
		currentConfig.Path = datadir
	}
...
func checkDeprecated(ctx *cli.Context) {
	...
	if ctx.GlobalString(DeprecatedsekeAPIFlag.Name) != "" {
...
func printConfig(config *bzzapi.Config) string {
	...
	if err != nil {
		return fmt.Sprintf("Somsekeing is not right with the configuration: %v", err)
	}
	return string(out)
}
