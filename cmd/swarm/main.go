...
import (
	...
	"github.com/sekechain/go-sekechain/cmd/utils"
	"github.com/sekechain/go-sekechain/common"
	"github.com/sekechain/go-sekechain/console"
	"github.com/sekechain/go-sekechain/crypto"
	"github.com/sekechain/go-sekechain/sekeclient"
	"github.com/sekechain/go-sekechain/internal/debug"
	...
	"github.com/sekechain/go-sekechain/swarm"
	bzzapi "github.com/sekechain/go-sekechain/swarm/api"
	swarmmetrics "github.com/sekechain/go-sekechain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the sekechain API provider to use to settle SWAP payments",
	...
	DeprecatedsekeAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside gseke.
func init() {
...
var app = utils.NewApp(gitCommit, "sekechain Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.sekeereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.sekeereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.sekeereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.sekeereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gseke only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gseke and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the sekeereum node
	...
	//register BZZ as node.Service in the sekeereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *sekeclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = sekeclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the sekeereum node
...
func getAccount(bzzaccount string, ctx *cli.Context, stack *node.Node) *ecdsa.PrivateKey {
	...
	return decrypsekeoreAccount(ks, bzzaccount, utils.MakePasswordList(ctx))
}

func decrypsekeoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
