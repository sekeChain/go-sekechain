...
import (
	...
	"github.com/sekechain/go-sekechain/internal/jsre"
	"github.com/sekechain/go-sekechain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute sekechain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jseke", struct{}{})

	jsekeObj, _ := c.jsre.Get("jseke")
	jsekeObj.Object().Set("send", bridge.Send)
	jsekeObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jseke);"); err != nil {
	...
	flatten := "var seke = web3.seke; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = seke.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jseke.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jseke.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jseke.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jseke.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jseke.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines seke.getBalance(seke.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gseke instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gseke metadata
	fmt.Fprintf(c.printer, "Welcome to the Gseke JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + seke.coinbase);
		console.log("at block: " + seke.blockNumber + " (" + new Date(1000 * seke.getBlock(seke.blockNumber).timestamp) + ")");
