...
func newTester(t *testing.T, confOverride func(*seke.Config)) *tester {
	...
	// Create a networkless protocol stack and start an sekechain service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	sekeConf := &seke.Config{
		Genesis:   core.DeveloperGenesisBlock(15, common.Address{}),
		sekeerbase: common.HexToAddress(testAddress),
		sekeash: ethash.Config{
			PowMode: ethash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(sekeConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return seke.New(ctx, sekeConf) }); err != nil {
		t.Fatalf("failed to register sekechain protocol: %v", err)
	}
