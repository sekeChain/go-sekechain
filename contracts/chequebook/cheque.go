...
// Package chequebook package wraps the 'chequebook' sekechain smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in sekeer; (auto)cashing cheques in sekeer
// as well as (auto)depositing sekeer to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/sekechain/go-sekechain/contracts/chequebook/contract"
	...
	"github.com/sekechain/go-sekechain/swarm/services/swap/swap"
)
...
// * depositing sekeer to the chequebook
// * watching incoming sekeer
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	_, ok := self.balance.Sesekering(file.Balance, 10)
	if !ok {
	...
	for addr, sent := range file.Sent {
		self.sent[common.HexToAddress(addr)], ok = new(big.Int).Sesekering(sent, 10)
		if !ok {
...
// Cash cashes the cheque by sending an sekechain transaction.
