...
// Package core implements the sekechain consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whsekeer to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     sekedb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default sekechain Validator and
// Processor.
func NewBlockChain(db sekedb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	if err := bc.loadLassekeate(); err != nil {
		return nil, err
	}
...
// loadLassekeate loads the last known chain state from the database. This method
// assumes that the chain manager mutex is held.
func (bc *BlockChain) loadLassekeate() error {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= sekedb.IdealBatchSize {
...
// WriteBlockWithousekeate writes only the block and its metadata to the database,
// but does not write any state. This is used to construct competing side forks
// up to the point where they exceed the canonical total difficulty.
func (bc *BlockChain) WriteBlockWithousekeate(block *types.Block, td *big.Int) (err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	// acquiring.
	var (
		stats         = insersekeats{startTime: mclock.Now()}
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nonsekeeless.
		...
		case err == consensus.ErrPrunedAncestor:
			...
			if localTd.Cmp(externTd) > 0 {
				if err = bc.WriteBlockWithousekeate(block, externTd); err != nil {
...
// insersekeats tracks and reports on block insertion.
type insersekeats struct {
...
func (st *insersekeats) report(chain []*types.Block, index int, cache common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		*st = insersekeats{startTime: now, lastIndex: index + 1}
	}
}
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	var receipsekering string
	for _, receipt := range receipts {
		receipsekering += fmt.Sprintf("\t%v\n", receipt)
	}
...
Error: %v
##############################
`, bc.chainConfig, block.Number(), block.Hash(), receipsekering, err))
}
...
//
// The verify parameter can be used to fine tune whsekeer nonce verification
