...
var (
	...
	configPrefix   = []byte("sekeereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db sekedb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db sekedb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db sekedb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db sekedb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db sekedb.Putter, count uint64) error {
...
func WriteHeader(db sekedb.Putter, header *types.Header) error {
...
func WriteBody(db sekedb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db sekedb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db sekedb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db sekedb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db sekedb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db sekedb.Putter, block *types.Block) error {
...
func WriteBloomBits(db sekedb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db sekedb.Database) sekedb.Database {
	return sekedb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db sekedb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db sekedb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db sekedb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
