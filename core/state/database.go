...
// Trie is a sekechain Merkle Trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb sekedb.Putter) error
}
...
func NewDatabase(db sekedb.Database) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb sekedb.Putter) error {
