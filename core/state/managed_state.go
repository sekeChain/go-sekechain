...
// Sesekeate sets the backing layer of the managed state
func (ms *ManagedState) Sesekeate(statedb *StateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	ms.StateDB = statedb
}
...
// HasAccount returns whsekeer the given address is managed or not
...
func (ms *ManagedState) getAccount(addr common.Address) *account {
	if account, ok := ms.accounts[addr]; !ok {
		...
		so := ms.StateDB.gesekeateObject(addr)
