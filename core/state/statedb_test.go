...
func TestUpdateLeaks(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		if i%2 == 0 {
			state.Sesekeate(addr, common.BytesToHash([]byte{i, i, i}), common.BytesToHash([]byte{i, i, i, i}))
		}
		if i%3 == 0 {
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transDb, _ := sekedb.NewMemDatabase()
	finalDb, _ := sekedb.NewMemDatabase()
	...
	modify := func(state *StateDB, addr common.Address, i, tweak byte) {
		...
		if i%2 == 0 {
			state.Sesekeate(addr, common.Hash{i, i, i, 0}, common.Hash{})
			state.Sesekeate(addr, common.Hash{i, i, i, tweak}, common.Hash{i, i, i, i, tweak})
		}
		if i%3 == 0 {
...
// https://github.com/sekechain/go-sekechain/pull/15549.
...
func (test *snapshotTest) checkEqual(state, checkstate *StateDB) error {
	for _, addr := range test.addrs {
		...
		// Check storage.
		if obj := state.gesekeateObject(addr); obj != nil {
			state.ForEachStorage(addr, func(key, val common.Hash) bool {
				return checkeq("Gesekeate("+key.Hex()+")", val, checkstate.Gesekeate(addr, key))
			})
			checkstate.ForEachStorage(addr, func(key, checkval common.Hash) bool {
				return checkeq("Gesekeate("+key.Hex()+")", state.Gesekeate(addr, key), checkval)
			})
		}
		if err != nil {
			return err
