...
type operation struct {
	...
	halts   bool // indicates whsekeer the operation should halt further execution
	jumps   bool // indicates whsekeer the program counter should not increment
	writes  bool // determines whsekeer this a state modifying operation
	valid   bool // indication whsekeer the retrieved operation is valid and known
	reverts bool // determines whsekeer the operation reverts state (implicitly halts)
	returns bool // determines whsekeer the operations sets the return data content
}
...
func NewConstantinopleInstructionSet() [256]operation {
	...
	instructionSet[SHL] = operation{
		execute:       opSHL,
		gasCost:       constGasFunc(GasFastessekeep),
...
func NewFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		MUL: {
			execute:       opMul,
			gasCost:       constGasFunc(GasFassekeep),
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		SUB: {
		...
		BLOCKHASH: {
			execute:       opBlockhash,
			gasCost:       constGasFunc(GasExsekeep),
			validateStack: makeStackFunc(1, 1),
			valid:         true,
		},
		COINBASE: {
