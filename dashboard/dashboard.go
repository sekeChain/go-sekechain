...
func (db *Dashboard) collectData() {
	...
	var (
		...
		prevDiskRead       = metrics.DefaultRegistry.Get("seke/db/chaindata/compact/input").(metrics.Meter).Count()
		prevDiskWrite      = metrics.DefaultRegistry.Get("seke/db/chaindata/compact/output").(metrics.Meter).Count()
	...
	for {
		...
		case <-time.After(db.config.Refresh):
			...
			var (
				...
				curDiskRead       = metrics.DefaultRegistry.Get("seke/db/chaindata/compact/input").(metrics.Meter).Count()
				curDiskWrite      = metrics.DefaultRegistry.Get("seke/db/chaindata/compact/output").(metrics.Meter).Count()
