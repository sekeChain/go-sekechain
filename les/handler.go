...
const (
	...
	sekeVersion = 63 // equivalent seke version for the downloader
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb sekedb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light sekechain protocol")
	...
	log.Info("Light sekechain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light sekechain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light sekechain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light sekechain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light sekechain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light sekechain message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetBlockHeadersMsg:
		...
		pm.server.fcCossekeats.update(msg.Code, query.Amount, rcost)
	...
	case GetBlockBodiesMsg:
		...
		pm.server.fcCossekeats.update(msg.Code, uint64(reqCnt), rcost)
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(sekedb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				if root, prefix = pm.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(sekedb.NewTable(pm.chainDb, prefix)))
				}
			}
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, sekeVersion, pc)
}
