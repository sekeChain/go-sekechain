...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db sekedb.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		bloomIndexer := seke.NewBloomIndexer(db, params.BloomBitsBlocks)
	...
	if !lightSync {
		...
		srv.fcCossekeats = newCossekeats(nil)
	}
	pm.Start(1000)
	return pm, nil
}
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db sekedb.Database) *ProtocolManager {
