...
import (
	...
	"github.com/sekechain/go-sekechain/les/flowcontrol"
...
type LesServer struct {
	...
	fcCossekeats     *requestCossekeats
...
func NewLesServer(seke *seke.sekechain, config *seke.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(seke.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, seke.EventMux(), seke.Engine(), newPeerSet(), seke.BlockChain(), seke.TxPool(), seke.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(seke.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(seke.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(seke.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(seke.BlockChain())
	pm.server = srv
	...
	srv.fcCossekeats = newCossekeats(seke.ChainDb())
	return srv, nil
}
...
type requestCossekeats struct {
	lock  sync.RWMutex
	db    sekedb.Database
	stats map[uint64]*linReg
}

type requestCossekeatsRlp []struct {
	MsgCode uint64
	Data    []byte
}

var rcStatsKey = []byte("_requestCossekeats")

func newCossekeats(db sekedb.Database) *requestCossekeats {
	...
	if db != nil {
		...
		var statsRlp requestCossekeatsRlp
		if err == nil {
	...
	return &requestCossekeats{
		db:    db,
		stats: stats,
	}
}

func (s *requestCossekeats) store() {
	s.lock.Lock()
	defer s.lock.Unlock()

	statsRlp := make(requestCossekeatsRlp, len(reqList))
...
func (s *requestCossekeats) getCurrentList() RequestCostList {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *requestCossekeats) update(msgCode, reqCnt, cost uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
