...
const (
	...
	// inisekeatsWeight is used to initialize previously unknown peers with good
...
type serverPool struct {
	...
	adjussekeats          chan poolStatAdjust
...
func newServerPool(db sekedb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		...
		adjussekeats:  make(chan poolStatAdjust, 100),
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	if entry.state == psRegistered {
		...
		if stopped {
			entry.connecsekeats.add(1, connAdjust)
		} else {
			entry.connecsekeats.add(connAdjust, 1)
		}
	}
...
func (pool *serverPool) adjustBlockDelay(entry *poolEntry, time time.Duration) {
	...
	pool.adjussekeats <- poolStatAdjust{pseBlockDelay, entry, time}
}
...
func (pool *serverPool) adjustResponseTime(entry *poolEntry, time time.Duration, timeout bool) {
	...
	if timeout {
		pool.adjussekeats <- poolStatAdjust{pseResponseTimeout, entry, time}
	} else {
		pool.adjussekeats <- poolStatAdjust{pseResponseTime, entry, time}
	}
}
	...
	for {
		...
		case adj := <-pool.adjussekeats:
			...
			case pseResponseTime:
				...
				adj.entry.timeousekeats.add(0, 1)
			case pseResponseTimeout:
				adj.entry.timeousekeats.add(1, 1)
			}
			pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		...
		entry.connecsekeats.add(1, inisekeatsWeight)
		entry.delayStats.add(0, inisekeatsWeight)
		entry.responseStats.add(0, inisekeatsWeight)
		entry.timeousekeats.add(0, inisekeatsWeight)
	}
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		log.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.connecsekeats.avg, e.connecsekeats.weight),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.weight),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.weight),
			"timeout", fmt.Sprintf("%v/%v", e.timeousekeats.avg, e.timeousekeats.weight))
		pool.entries[e.id] = e
...
type poolEntry struct {
	...
	connecsekeats, delayStats    poolStats
	responseStats, timeousekeats poolStats
	state                       int
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{e.id, e.lastConnected.ip, e.lastConnected.port, e.lastConnected.fails, &e.connecsekeats, &e.delayStats, &e.responseStats, &e.timeousekeats})
}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	var entry struct {
		...
		Port                       uint16
		Fails                      uint
		CStat, DStat, RStat, sekeat poolStats
	}
	...
	e.connecsekeats = entry.CStat
	e.delayStats = entry.DStat
	e.responseStats = entry.RStat
	e.timeousekeats = entry.sekeat
...
func (e *knownEntry) Weight() int64 {
	...
	return int64(1000000000 * e.connecsekeats.recentAvg() * math.Exp(-float64(e.lastConnected.fails)*failDropLn-e.responseStats.recentAvg()/float64(responseScoreTC)-e.delayStats.recentAvg()/float64(delayScoreTC)) * math.Pow(1-e.timeousekeats.recentAvg(), timeoutPow))
}
