...
import (
	...
	"github.com/sekechain/go-sekechain/common/bitutil"
...
func GetChtRoot(db sekedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db sekedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(CHTFrequencyClient/CHTFrequencyServer)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db sekedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db sekedb.Database, clientMode bool) *core.ChainIndexer {
	...
	idb := sekedb.NewTable(db, "chtIndex-")
	backend := &ChtIndexerBackend{
		diskdb:      db,
		triedb:      trie.NewDatabase(sekedb.NewTable(db, ChtTablePrefix)),
...
const (
	...
	sekeBloomBitsSection       = 4096
	sekeBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db sekedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db sekedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db sekedb.Database, clientMode bool) *core.ChainIndexer {
	backend := &BloomTrieIndexerBackend{
		diskdb: db,
		triedb: trie.NewDatabase(sekedb.NewTable(db, BloomTrieTablePrefix)),
	}
	idb := sekedb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = sekeBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-sekeBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
