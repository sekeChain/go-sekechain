...
// to the seke network. The implementations of the functions should be non-blocking.
//
...
// currensekeate returns the light state of the current head header
func (pool *TxPool) currensekeate(ctx context.Context) *state.StateDB {
...
// sesekeate sets the status of a tx to either recently mined or recently rolled back
func (txc txStateChanges) sesekeate(txHash common.Hash, mined bool) {
	val, ent := txc[txHash]
	if ent && (val != mined) {
		delete(txc, txHash)
	} else {
...
// validateTx checks whsekeer a transaction is valid according to the consensus rules.
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	currensekeate := pool.currensekeate(ctx)
	if n := currensekeate.GetNonce(from); n > tx.Nonce() {
	...
	// cost == V + GP * GL
	if b := currensekeate.GetBalance(from); b.Cmp(tx.Cost()) < 0 {
