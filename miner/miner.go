...
// Package miner implements sekechain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whsekeer we can start the mining operation
	shouldStart int32 // should start indicates whsekeer we should start after sync
}

func New(seke Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		seke:      seke,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, seke, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(seke.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setsekeerbase(coinbase)
...
func (self *Miner) Setsekeerbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setsekeerbase(addr)
}
