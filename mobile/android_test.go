...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gseke.LightScryptN, Gseke.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.sekeereum", new NodeConfig());
			node.start();
			...
			sekechainClient ec = node.getsekechainClient();
	...
	// signatures too. Regression test for go-sekeereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogsekeer
	...
	workspace, err := ioutil.TempDir("", "gseke-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/sekeereum/gseketest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gseke and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.sekeereum", "github.com/sekechain/go-sekechain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gseke.aar"), "gseke.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "sekeereum", "gseketest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.sekeereum.gseketest"
