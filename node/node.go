...
func (n *Node) OpenDatabase(name string, cache, handles int) (sekedb.Database, error) {
	if n.config.DataDir == "" {
		return sekedb.NewMemDatabase()
	}
	return sekedb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
