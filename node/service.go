...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (sekedb.Database, error) {
	if ctx.config.DataDir == "" {
		return sekedb.NewMemDatabase()
	}
	db, err := sekedb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
