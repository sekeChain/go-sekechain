...
import (
	...
	"github.com/sekechain/go-sekechain/crypto/sha3"
...
func (net *Network) SetFallbackNodes(nodes []*Node) error {
	...
	for _, n := range nodes {
		if err := n.validateComplete(); err != nil {
			return fmt.Errorf("bad boosekerap/fallback node %q (%v)", n, err)
		}
...
func (net *Network) loop() {
	...
	resetNextTicket := func() {
		ticket, timeout := net.tickesekeore.nextFilteredTicket()
...
loop:
	for {
		...
		case req := <-net.topicRegisterReq:
			...
			if !req.add {
				net.tickesekeore.removeRegisterTopic(req.topic)
				continue
			}
			net.tickesekeore.addTopic(req.topic, true)
			...
			if topicRegisterLookupTarget.target == (common.Hash{}) {
				...
				target, delay := net.tickesekeore.nextRegisterLookup()
		...
		case nodes := <-topicRegisterLookupDone:
			...
			net.tickesekeore.registerLookupDone(topicRegisterLookupTarget, nodes, func(n *Node) []byte {
				net.ping(n, n.addr())
				return n.pingEcho
			})
			target, delay := net.tickesekeore.nextRegisterLookup()
		...
		case <-nextRegisterTime:
			...
			net.tickesekeore.ticketRegistered(*nextTicket)
			...
			if refreshDone == nil {
				...
				if ok {
					if req.delay == time.Duration(0) {
						...
						net.tickesekeore.removeSearchTopic(req.topic)
					} else {
				...
				if req.delay != time.Duration(0) {
					...
					searchInfo[req.topic] = info
					net.tickesekeore.addSearchTopic(req.topic, req.found)
		...
		case topic := <-topicSearch:
			if activeSearchCount < maxSearchCount {
				activeSearchCount++
				target := net.tickesekeore.nextSearchLookup(topic)
				go func() {
		...
		case res := <-topicSearchLookupDone:
			...
			if lookupChn := searchInfo[res.target.topic].lookupChn; lookupChn != nil {
				lookupChn <- net.tickesekeore.radius[res.target.topic].converged
			}
			net.tickesekeore.searchLookupDone(res.target, res.nodes, func(n *Node, topic Topic) []byte {
		...
		case <-statsDump.C:
			...
			/*r, ok := net.tickesekeore.radius[testTopic]
			if !ok {
				fmt.Printf("(%x) no radius @ %v\n", net.tab.self.ID[:8], time.Now())
			} else {
				topics := len(net.tickesekeore.tickets)
				tickets := len(net.tickesekeore.nodes)
				rad := r.radius / (maxRadius/10000+1)
				fmt.Printf("(%x) topics:%d radius:%d tickets:%d @ %v\n", net.tab.self.ID[:8], topics, rad, tickets, time.Now())
			}*/

			tm := mclock.Now()
			for topic, r := range net.tickesekeore.radius {
				if printTestImgLogs {
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	for _, n := range seeds {
		...
		// Force-add the seed node so Lookup does somsekeing.
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	n.pingTopics = net.tickesekeore.regTopicSet()
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	...
	if err == nil {
		...
		net.tickesekeore.addTicket(now, pkt.data.(*pong).ReplyTok, ticket)
	} else {
...
func (net *Network) handleQueryEvent(n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
	...
	case topicQueryPacket:
		...
		if _, ok := net.tickesekeore.tickets[topic]; ok {
	...
	case topicNodesPacket:
		...
		if net.tickesekeore.gotTopicNodes(n, p.Echo, p.Nodes) {
			n.queryTimeouts++
			if n.queryTimeouts > maxFindnodeFailures && n.state == known {
				return contested, errors.New("too many timeouts")
			}
