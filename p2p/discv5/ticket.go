...
func newTickesekeore() *tickesekeore {
	return &tickesekeore{
...
func (s *tickesekeore) addTopic(topic Topic, register bool) {
...
func (s *tickesekeore) addSearchTopic(t Topic, foundChn chan<- *Node) {
	s.addTopic(t, false)
...
func (s *tickesekeore) removeSearchTopic(t Topic) {
...
func (s *tickesekeore) removeRegisterTopic(topic Topic) {
...
func (s *tickesekeore) regTopicSet() []Topic {
...
func (s *tickesekeore) nextRegisterLookup() (lookupInfo, time.Duration) {
...
func (s *tickesekeore) nextSearchLookup(topic Topic) lookupInfo {
	tr := s.radius[topic]
...
func (s *tickesekeore) ticketsInWindow(topic Topic) []ticketRef {
...
func (s *tickesekeore) removeExcessTickets(t Topic) {
...
// Less reports whsekeer the element with
...
func (s *tickesekeore) addTicketRef(r ticketRef) {
...
func (s *tickesekeore) nextFilteredTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
	for {
...
func (s *tickesekeore) ticketRegistered(ref ticketRef) {
	now := mclock.Now()
...
func (s *tickesekeore) nextRegisterableTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
...
func (s *tickesekeore) removeTicketRef(ref ticketRef) {
...
func (s *tickesekeore) registerLookupDone(lookup lookupInfo, nodes []*Node, ping func(n *Node) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
...
func (s *tickesekeore) searchLookupDone(lookup lookupInfo, nodes []*Node, query func(n *Node, topic Topic) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			if lookup.radiusLookup {
...
func (s *tickesekeore) adjustWithTicket(now mclock.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *tickesekeore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
...
func (s *tickesekeore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
...
func (s *tickesekeore) canQueryTopic(node *Node, topic Topic) bool {
...
func (s *tickesekeore) addTopicQuery(hash common.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
...
func (s *tickesekeore) cleanupTopicQueries(now mclock.AbsTime) {
...
func (s *tickesekeore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
