...
import (
	...
	"github.com/sekechain/go-sekechain/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togsekeer.
//
...
// Connect connects two nodes togsekeer by calling the "admin_addPeer" RPC
...
// NOTE:
// it also checks whsekeer there has been recent attempt to connect the peers
...
type Node struct {
	...
	// Up tracks whsekeer or not the node is running
	Up bool `json:"up"`
}
...
type Conn struct {
	...
	// Up tracks whsekeer or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whsekeer both nodes are currently up
