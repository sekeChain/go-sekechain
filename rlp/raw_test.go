...
func TestSplitTypes(t *testing.T) {
	if _, _, err := Splisekering(unhex("C100")); err != ErrExpectedString {
		t.Errorf("Splisekering returned %q, want %q", err, ErrExpectedString)
	}
	if _, _, err := SplitList(unhex("01")); err != ErrExpectedList {
		t.Errorf("Splisekering returned %q, want %q", err, ErrExpectedList)
	}
