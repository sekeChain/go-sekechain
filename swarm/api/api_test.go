...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.seke"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.seke"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.seke"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	sekeAddr := "swarm.seke"
	sekeHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	sekeResolve := newTestResolver(sekeHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(sekeResolve, "")),
			addr:   sekeAddr,
			result: sekeHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(sekeResolve, ""),
				MultiResolverOptionWithResolver(sekeResolve, ""),
			),
			addr:   sekeAddr,
			result: sekeHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(sekeResolve, "seke"),
			),
			addr:   sekeAddr,
			result: sekeHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "seke"),
				MultiResolverOptionWithResolver(doesntResolve, "seke"),
				MultiResolverOptionWithResolver(sekeResolve, "seke"),
			),
			addr:   sekeAddr,
		...
		},
		{
			...
			addr:   sekeAddr,
			result: sekeHash,
			err:    errors.New(`DNS name not found: "swarm.seke"`),
		},
		{
