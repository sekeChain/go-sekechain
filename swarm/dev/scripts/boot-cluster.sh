...
#
# The cluster contains a bootnode, a gseke node and multiple swarm nodes, with
...
# static gseke configuration
Gseke_IP="192.168.33.3"
Gseke_RPC_PORT="8545"
Gseke_RPC_URL="http://${Gseke_IP}:${Gseke_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to boosekerap the gseke and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gseke node with --datadir pointing at <base-dir>/gseke
# and a single, unlocked account with password "gseke"
start_geth_node() {
  local dir="${base_dir}/gseke"
  mkdir -p "${dir}"

  local password="gseke"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating gseke account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gseke account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Gseke_IP}"
    --rpcport   "${Gseke_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gseke" "${Gseke_IP}" "$(which gseke)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vseke pair
  local vseke0="vseke${name}0"
  local vseke1="vseke${name}1"
  ip link add name "${vseke0}" type vseke peer name "${vseke1}"

  # add one end to the bridge
  ip link set dev "${vseke0}" master "${BRIDGE_NAME}"
  ip link set dev "${vseke0}" up

  # add the other end to the namespace, rename it seke0 and give it the ip
  ip link set dev "${vseke1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vseke1}" name "seke0"
  ip netns exec "${name}" ip link set dev "seke0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "seke0"
}
...
create_account() {
  ...
  gseke --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
