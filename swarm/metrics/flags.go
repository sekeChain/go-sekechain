...
import (
	...
	gsekemetrics "github.com/sekechain/go-sekechain/metrics"
	"github.com/sekechain/go-sekechain/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if gsekemetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gsekemetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
