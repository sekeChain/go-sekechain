...
func NewDepo(hash storage.SwarmHasher, localStore, remoteStore storage.ChunkStore) *Depo {
	return &Depo{
		...
		nesekeore:   remoteStore, // entrypoint internal
	}
}
...
// remote is untrusted, so hash is verified and chunk passed on to Nesekeore
...
func (self *Depo) HandleRetrieveRequestMsg(req *retrieveRequestMsgData, p *peer) {
	...
	// call storage.Nesekeore#Get which
	// blocks until local retrieval finished
	// launches cloud retrieval
	chunk, _ := self.nesekeore.Get(req.Key)
...
func (self *Depo) strategyUpdateRequest(rs *storage.Requessekeatus, origReq *retrieveRequestMsgData) (req *retrieveRequestMsgData) {
...
func (self *Depo) searchTimeout(rs *storage.Requessekeatus, req *retrieveRequestMsgData) (timeout *time.Time) {
	reqt := req.getTimeout()
...
*/
func (self *Depo) addRequester(rs *storage.Requessekeatus, req *retrieveRequestMsgData) {
