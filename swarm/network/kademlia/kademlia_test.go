...
var (
	...
	quickcfgBoosekerap   = &quick.Config{MaxCount: 100, Rand: quickrand}
)
...
func TestBoosekerap(t *testing.T) {

	test := func(test *boosekerapTest) bool {
	...
	if err := quick.Check(test, quickcfgBoosekerap); err != nil {
		t.Error(err)
	}
...
type boosekerapTest struct {
	MaxProx    int
	BucketSize int
	Self       Address
}

func (*boosekerapTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &boosekerapTest{
