...
// Chunk retrieval blocks on nesekeore requests with a timeout so reader will
...
// retrieve channel to its ChunkStore  (Nesekeore or LocalStore)
...
// received on the store channel to its ChunkStore (Nesekeore or LocalStore)
...
// local storage eg. LocalStore and network storage eg., Nesekeore
// access by calling network is blocking with a timeout

type dpaChunkStore struct {
	n          int
	localStore ChunkStore
	nesekeore   ChunkStore
}

func NewDpaChunkStore(localStore, nesekeore ChunkStore) *dpaChunkStore {
	return &dpaChunkStore{0, localStore, nesekeore}
}

// Get is the entrypoint for local retrieve requests
// waits for response or times out
func (self *dpaChunkStore) Get(key Key) (chunk *Chunk, err error) {
	chunk, err = self.nesekeore.Get(key)
...
func (self *dpaChunkStore) Put(entry *Chunk) {
	...
	if err != nil {
		log.Trace(fmt.Sprintf("DPA.Put: %v new chunk. call nesekeore.Put", entry.Key.Log()))
		chunk = entry
