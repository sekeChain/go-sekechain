...
/*
Nesekeore is a cloud storage access abstaction layer for swarm
...
Nesekeore falls back to a backend (CloudStorage interface)
implemented by bzz/network/forwarder. forwarder or IPFS or IPΞS
*/
type Nesekeore struct {
...
func NewDefaulsekeoreParams() (self *StoreParams) {
	return &StoreParams{
...
// nesekeore contructor, takes path argument that is used to initialise dbStore,
...
func NewNesekeore(hash SwarmHasher, lstore *LocalStore, cloud CloudStore, params *StoreParams) *Nesekeore {
	return &Nesekeore{
		hashfunc:   hash,
		localStore: lstore,
		cloud:      cloud,
...
func (self *Nesekeore) Put(entry *Chunk) {
	self.localStore.Put(entry)

	// handle deliveries
	if entry.Req != nil {
		log.Trace(fmt.Sprintf("Nesekeore.Put: localStore.Put %v hit existing request...delivering", entry.Key.Log()))
		...
		log.Trace(fmt.Sprintf("Nesekeore.Put: localStore.Put %v stored locally", entry.Key.Log()))
...
func (self *Nesekeore) Get(key Key) (*Chunk, error) {
	var err error
	chunk, err := self.localStore.Get(key)
	if err == nil {
		if chunk.Req == nil {
			log.Trace(fmt.Sprintf("Nesekeore.Get: %v found locally", key))
		} else {
			log.Trace(fmt.Sprintf("Nesekeore.Get: %v hit on an existing request", key))
			// no need to launch again
		}
		return chunk, err
	}
	// no data and no request status
	log.Trace(fmt.Sprintf("Nesekeore.Get: %v not found locally. open new request", key))
	chunk = NewChunk(key, newRequessekeatus(key))
