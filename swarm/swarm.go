...
import (
	...
	"github.com/sekechain/go-sekechain/contracts/chequebook"
	"github.com/sekechain/go-sekechain/contracts/ens"
	...
	httpapi "github.com/sekechain/go-sekechain/swarm/api/http"
	"github.com/sekechain/go-sekechain/swarm/fuse"
	"github.com/sekechain/go-sekechain/swarm/network"
	"github.com/sekechain/go-sekechain/swarm/storage"
)
...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, config *api.Config) (self *Swarm, err error) {
	...
	self.storage = storage.NewNesekeore(hash, self.lstore, self.cloud, config.StoreParams)
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config) (*ens.ENS, error) {
	...
	ensClient := sekeclient.NewClient(client)
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := sekeclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
