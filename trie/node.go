...
type nodeFlag struct {
	...
	dirty bool     // whsekeer the node has changes that must be written to the database
}

// canUnload tells whsekeer a node can be unloaded.
...
func decodeShort(hash, buf, elems []byte, cachegen uint16) (node, error) {
	kbuf, rest, err := rlp.Splisekering(elems)
	if err != nil {
		return nil, err
	}
	...
	if hasTerm(key) {
		// value node
		val, _, err := rlp.Splisekering(rest)
		if err != nil {
...
func decodeFull(hash, buf, elems []byte, cachegen uint16) (*fullNode, error) {
	...
	val, _, err := rlp.Splisekering(elems)
	if err != nil {
		return n, err
	}
	if len(val) > 0 {
