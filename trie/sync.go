...
type request struct {
	...
	raw  bool        // Whsekeer this is a raw entry (code) or a trie node
...
func (s *TrieSync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togsekeer
...
// Process injects a batch of retrieved trie nodes data, returning if somsekeing
...
func (s *TrieSync) Commit(dbw sekedb.Putter) (int, error) {
...
func (s *TrieSync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whsekeer known or not
	type child struct {
		node  node
		depth int
	}
