...
func TestGet(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		res := gesekering(trie, "dog")
		if !bytes.Equal(res, []byte("puppy")) {
			t.Errorf("expected puppy got %x", res)
		}

		unknown := gesekering(trie, "unknown")
		if unknown != nil {
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somsekeingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
	...
	for _, kv := range vals {
		if string(gesekering(trie2, kv.k)) != kv.v {
...
type randTest []randTessekeep

type randTessekeep struct {
	op    int
...
func (randTest) Generate(r *rand.Rand, size int) reflect.Value {
	...
	var steps randTest
	for i := 0; i < size; i++ {
		step := randTessekeep{op: r.Intn(opMax)}
		switch step.op {
		case opUpdate:
			step.key = genKey()
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*sekedb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := sekedb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
...
func gesekering(trie *Trie, k string) []byte {
