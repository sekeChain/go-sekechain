...
// stateReq represents a batch of state fetch requests groupped togsekeer into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whsekeer the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somsekeing else, ignore the stale timeout.
...
func (s *stateSync) loop() error {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somsekeing to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < sekedb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) process(req *stateReq) error {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somsekeing, missing items may be due to a protocol
...
// peer into the state trie, returning whsekeer anything useful was written or any
// error occurred.
