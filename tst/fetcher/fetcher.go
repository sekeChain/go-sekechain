...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (seke/61) or header (seke/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (seke/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both seke/61 and seke/62)
}
...
func (f *Fetcher) loop() {
	...
	for {
		...
		case notification := <-f.notify:
			...
			if count > hashLimit {
				log.Debug("Peer exceeded ousekeanding announces", "peer", notification.origin, "limit", hashLimit)
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somsekeing went very wrong, drop the peer
