...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/gseke/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := sekedb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = sekedb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db sekedb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*sekedb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db sekedb.Database) {
