...
// Package filters implements an sekeereum filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  sekeereum.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit sekeereum.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit sekeereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit sekeereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit sekeereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
