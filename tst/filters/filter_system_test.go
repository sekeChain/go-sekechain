...
import (
	...
	sekeereum "github.com/sekechain/go-sekechain"
...
func (b *testBackend) ChainDb() sekedb.Database {
	return b.db
}
...
// TestPendingTxFilter tests whsekeer pending tx filters retrieve all pending transactions that are posted to the event mux.
...
// TestLogFilterCreation test whsekeer a given filter criteria makes sense.
...
// TestInvalidLogFilterCreation tests whsekeer invalid filter log criteria results in an error
...
// TestLogFilter tests whsekeer log filters match the correct logs that are posted to the event feed.
...
func TestPendingLogsSubscription(t *testing.T) {
	...
	var (
		...
		testCases = []struct {
			crit     sekeereum.FilterQuery
			expected []*types.Log
			c        chan []*types.Log
			sub      *Subscription
		}{
			// match all
			{sekeereum.FilterQuery{}, convertLogs(allLogs), nil, nil},
			// match none due to no matching addresses
			{sekeereum.FilterQuery{Addresses: []common.Address{{}, notUsedAddress}, Topics: [][]common.Hash{nil}}, []*types.Log{}, nil, nil},
			// match logs based on addresses, ignore topics
			{sekeereum.FilterQuery{Addresses: []common.Address{firstAddr}}, append(convertLogs(allLogs[:2]), allLogs[5].Logs[3]), nil, nil},
			// match none due to no matching topics (match with address)
			{sekeereum.FilterQuery{Addresses: []common.Address{secondAddr}, Topics: [][]common.Hash{{notUsedTopic}}}, []*types.Log{}, nil, nil},
			// match logs based on addresses and topics
			{sekeereum.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}}, append(convertLogs(allLogs[3:5]), allLogs[5].Logs[0]), nil, nil},
			// match logs based on multiple addresses and "or" topics
			{sekeereum.FilterQuery{Addresses: []common.Address{secondAddr, thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}}, append(convertLogs(allLogs[2:5]), allLogs[5].Logs[0]), nil, nil},
			// block numbers are ignored for filters created with New***Filter, these return all logs that match the given criteria when the state changes
			{sekeereum.FilterQuery{Addresses: []common.Address{firstAddr}, FromBlock: big.NewInt(2), ToBlock: big.NewInt(3)}, append(convertLogs(allLogs[:2]), allLogs[5].Logs[3]), nil, nil},
			// multiple pending logs, should match only 2 topics from the logs in block 5
			{sekeereum.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, fourthTopic}}}, []*types.Log{allLogs[5].Logs[0], allLogs[5].Logs[2]}, nil, nil},
		}
	)
