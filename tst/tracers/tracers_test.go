...
import (
	...
	"github.com/sekechain/go-sekechain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gseke console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = seke.getBlock(seke.getTransaction(tx).blockHash);
  var genesis = seke.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.seke.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  seke.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
