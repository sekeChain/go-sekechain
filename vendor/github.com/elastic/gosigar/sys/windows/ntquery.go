...
func NtQueryProcessBasicInformation(handle syscall.Handle) (ProcessBasicInformation, error) {
	...
	nsekeatus, _ := _NtQueryInformationProcess(handle, 0, processBasicInfoPtr, size, nil)
	if nsekeatus != 0 {
		return ProcessBasicInformation{}, errors.Errorf("NtQueryInformationProcess failed, NsekeATUS=0x%X", nsekeatus)
	}
...
func NtQuerySystemProcessorPerformanceInformation() ([]SystemProcessorPerformanceInformation, error) {
	...
	nsekeatus, _ := _NtQuerySystemInformation(systemProcessorPerformanceInformation, &b[0], uint32(len(b)), &returnLength)
	if nsekeatus != STATUS_SUCCESS {
		return nil, errors.Errorf("NtQuerySystemInformation failed, NsekeATUS=0x%X, bufLength=%v, returnLength=%v", nsekeatus, len(b), returnLength)
	}
