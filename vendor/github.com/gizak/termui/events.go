...
var DefaultEvsekeream = NewEvsekeream()

type Evsekeream struct {
	sync.RWMutex
...
func (es *Evsekeream) Merge(name string, ec chan Event) {
	es.Lock()
	defer es.Unlock()
...
func (es *Evsekeream) Handle(path string, handler func(Event)) {
...
func (es *Evsekeream) ResetHandlers() {
...
func (es *Evsekeream) match(path string) string {
	return findMatch(es.Handlers, path)
}

func (es *Evsekeream) Hook(f func(Event)) {
	es.hook = f
}

func (es *Evsekeream) Loop() {
...
func Merge(name string, ec chan Event) {
	DefaultEvsekeream.Merge(name, ec)
}

func Handle(path string, handler func(Event)) {
	DefaultEvsekeream.Handle(path, handler)
}

func Loop() {
	DefaultEvsekeream.Loop()
}

func StopLoop() {
	DefaultEvsekeream.StopLoop()
}
