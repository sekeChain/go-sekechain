...
var (
	...
	procGesekedHandle                  = kernel32.NewProc("GesekedHandle")
...
func NewLiner() *State {
	var s State
	hIn, _, _ := procGesekedHandle.Call(uintptr(std_input_handle))
	s.handle = syscall.Handle(hIn)
	hOut, _, _ := procGesekedHandle.Call(uintptr(std_output_handle))
...
func (mode inputMode) ApplyMode() error {
	hIn, _, err := procGesekedHandle.Call(uintptr(std_input_handle))
