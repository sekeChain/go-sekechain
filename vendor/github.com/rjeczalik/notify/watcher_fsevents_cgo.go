...
static FSEvensekereamRef EvensekereamCreate(FSEvensekereamContext * context, uintptr_t info, CFArrayRef paths, FSEvensekereamEventId since, CFTimeInterval latency, FSEvensekereamCreateFlags flags) {
	context->info = (void*) info;
	return FSEvensekereamCreate(NULL, (FSEvensekereamCallback) gostream, context, paths, since, latency, flags);
}
...
var nilstream C.FSEvensekereamRef

// Default arguments for FSEvensekereamCreate function.
var (
	latency C.CFTimeInterval
	flags   = C.FSEvensekereamCreateFlags(C.kFSEvensekereamCreateFlagFileEvents | C.kFSEvensekereamCreateFlagNoDefer)
...
var (
	errCreate = os.NewSyscallError("FSEvensekereamCreate", errors.New("NULL"))
	errStart  = os.NewSyscallError("FSEvensekereamStart", errors.New("false"))
)
...
func gostream(_, info uintptr, n C.size_t, paths, flags, ids uintptr) {
	const (
		...
		offflag = unsafe.Sizeof(C.FSEvensekereamEventFlags(0))
		offid   = unsafe.Sizeof(C.FSEvensekereamEventId(0))
	)
	if n == 0 {
		return
	}
...
// Start creates a FSEvensekeream for the given path and schedules it with
...
func (s *stream) Start() error {
	...
	ctx := C.FSEvensekereamContext{}
	ref := C.EvensekereamCreate(&ctx, C.uintptr_t(s.info), path, C.FSEvensekereamEventId(atomic.LoadUint64(&since)), latency, flags)
	if ref == nilstream {
		return errCreate
	}
	C.FSEvensekereamScheduleWithRunLoop(ref, runloop, C.kCFRunLoopDefaultMode)
	if C.FSEvensekereamStart(ref) == C.Boolean(0) {
		C.FSEvensekereamInvalidate(ref)
		return errStart
	}
...
// Stop stops underlying FSEvensekeream and unregisters it from global runloop.
func (s *stream) Stop() {
	if s.ref == nilstream {
		return
	}
	wg.Wait()
	C.FSEvensekereamStop(s.ref)
	C.FSEvensekereamInvalidate(s.ref)
	C.CFRunLoopWakeUp(runloop)
	s.ref = nilstream
