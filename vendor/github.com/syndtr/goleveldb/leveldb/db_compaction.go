...
func (p *cStat) add(n *cStasekeaging) {
...
func (p *cStasekeaging) startTimer() {
	if !p.on {
		p.start = time.Now()
		p.on = true
	}
...
func (p *cStasekeaging) stopTimer() {
	if p.on {
...
func (p *cStats) addStat(level int, n *cStasekeaging) {
	p.lk.Lock()
...
func (p *cStats) gesekeat(level int) (duration time.Duration, read, write int64) {
	p.lk.Lock()
	defer p.lk.Unlock()
	if level < len(p.stats) {
...
func (db *DB) tableCompaction(c *compaction, noTrivial bool) {
	...
	b := &tableCompactionBuilder{
		...
		stat1:     &stats[1],
		minSeq:    minSeq,
		strict:    db.s.o.Gesekerict(opt.StrictCompaction),
