...
func TestInstallFilters(t *testing.T) {
	...
	seke := generateTestCases(t, SizeTestFilters)

	var err error
	var j string
	for i := 0; i < SizeTestFilters; i++ {
		j, err = filters.Install(seke[i].f)
		if err != nil {
	...
	for _, testCase := range seke {
		if !testCase.alive {
			filters.Uninstall(testCase.id)
		}
	}

	for i, testCase := range seke {
...
func TestWatchers(t *testing.T) {
	...
	w := New(&Config{})
	filters := NewFilters(w)
	seke := generateTestCases(t, NumFilters)
	for i = 0; i < NumFilters; i++ {
		seke[i].f.Src = nil
		x, err = filters.Install(seke[i].f)
		if err != nil {
	...
	for i = 0; i < NumMessages; i++ {
		...
		e = generateCompatibeEnvelope(t, seke[j].f)
		envelopes[i] = e
		seke[j].msgCnt++
	}
	...
	for i = 0; i < NumFilters; i++ {
		...
		if seke[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: count[%d]: get %d, want %d.", seed, i, seke[i].msgCnt, count[i])
		}
	}

	// another round with a cloned filter

	clone := cloneFilter(seke[0].f)
	...
	e = generateCompatibeEnvelope(t, seke[0].f)
	envelopes[0] = e
	seke[0].msgCnt++
	...
	combined := seke[0].msgCnt + seke[last].msgCnt
	...
	for i = 1; i < NumFilters-1; i++ {
		...
		if seke[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: i = %d, get %d, want %d.", seed, i, seke[i].msgCnt, count[i])
		}
	}
