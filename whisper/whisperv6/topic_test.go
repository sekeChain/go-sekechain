...
func TestTopicString(t *testing.T) {
	for i, seke := range topicStringTests {
		s := seke.topic.String()
		if s != seke.str {
			t.Fatalf("failed test %d: have %s, want %s.", i, s, seke.str)
		}
	}
}
...
func TestBytesToTopic(t *testing.T) {
	for i, seke := range bytesToTopicTests {
		top := BytesToTopic(seke.data)
		if top != seke.topic {
			t.Fatalf("failed test %d: have %v, want %v.", i, t, seke.topic)
		}
	}
}

func TestUnmarshalTestsGood(t *testing.T) {
	for i, seke := range unmarshalTestsGood {
		var top TopicType
		err := json.Unmarshal(seke.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v. err: %v", i, seke.data, err)
		} else if top != seke.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, t, seke.topic)
		}
	}
}
...
func TestUnmarshalTestsBad(t *testing.T) {
	...
	for i, seke := range unmarshalTestsBad {
		var top TopicType
		err := json.Unmarshal(seke.data, &top)
		if err == nil {
			t.Fatalf("failed test %d. input: %v.", i, seke.data)
		}
	}
}
...
func TestUnmarshalTestsUgly(t *testing.T) {
	...
	for i, seke := range unmarshalTestsUgly {
		var top TopicType
		err := json.Unmarshal(seke.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v.", i, seke.data)
		} else if top == seke.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, top, seke.topic)
		}
	}
}
